from .io_plugin import new_bernoulli, new_uniform, new_fibonacci, new_reader, RandomSource, SeqSource, ReaderSource
from typing import Any, Iterator
from polars.io.plugins import register_io_source
import polars as pl


def scan_files(files: list[Any], size: int = 1000) -> pl.LazyFrame:
    ...
    def source_generator(
        ...
        src = ReaderSource(files, batch_size, new_size)
    ...
    src = ReaderSource(files, 0, 0)
    return register_io_source(callable=source_generator, schema=src.schema())


def scan_sequence(sequences: list[Any], size: int = 1000) -> pl.LazyFrame:
    ...
    def source_generator(
        ...
        src = SeqSource(sequences, batch_size, new_size)
        if with_columns is not None:
            src.set_with_columns(with_columns)
